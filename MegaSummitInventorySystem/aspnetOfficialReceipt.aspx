...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function SaveOfficialReceipt()
        {
            var cash = $('#input_cash_amount').val();
            ...
            $("#tbl_apply_invoices tbody tr").find('td').each(function () {
                ...
                {
                    apply_for +=  Number($(this).find(".apply").val()) ;
                }
            });
        ...
        function CheckCollection()
        {
            ...
            $("#check tbody tr").find('td').each(function () {
                ...
                {
                    col +=  ($(this).find(".bank").val()) + '^' + ($(this).find(".check_no").val()) + '^' + ($(this).find("._date").val())+ '^' + ($(this).find(".amount").val()) + '|';
                }
            });
        ...
        function FillCustomer(customer_id)
        {
            ...
            $.ajax({
                type: "POST", url: pageUrl + "/GetOfficialReceiptCustomer", data: "{'id':'0', 'invoice_id':'0', 'customer_id':'" + customer_id + "' }",
                contentType: "application/json; charset=utf-8", dataType: "json", crossdomain: true,
                success: function (response) {
                    ...
                    var data = eval(response.d);
                    for (var i = 0; i < data.length; i++) {
                        $("#ulCustomer").append(ListOfficialReceipt(data[i]['CustomerName'],data[i]['RefNo'],data[i]['Amount']) );
                    }

                    for (var i = 0; i < 20; i++) {
                        $("#ulCustomer").append(ListOfficialReceipt('&nbsp;','&nbsp;','0') );
                    }
                },
                error: function (response) {
                }
